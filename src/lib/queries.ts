...
export const saveActivityLogsNotification = async ({
    ...
    if(!foundAgencyId) {
        if(!subaccountId) {
            throw new Error('You need to provide atleast an agency Id or subaccount ')
        }
...
export const createTeamUser = async (agenctId:string, user:User) => {
...
export const updateAgencyDetails = async (agencyId:string, agencyDetails: Partial<Agency>) => {
    const response = await db.agency.update({where:{id:agencyId},
         data: {...agencyDetails}})
    return response
}
